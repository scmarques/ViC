...
import com.sephora.moviesapp.presentation.viewmodels.FavoriteMoviesViewModel
...
class FavoriteMoviesFragment : Fragment(R.layout.fragment_favorite_movies) {
    ...
    private fun showMovieDetails(movie: DetailedMovieEntity?) {
        movie?.let {
            val action =
                CollectionFragmentDirections.actionCollectionFragmentToMovieDetailFragment(
                    movieId = movie.movieId, local = "local"
                )
    ...
    /*
       fun setupObserveMoviesByGenre(genre: GenreModel) {
        viewModel.getMoviesByCategory(genre).observe(viewLifecycleOwner, {
